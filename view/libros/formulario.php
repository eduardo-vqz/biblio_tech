            ...
            <div class="mb-3">
                ...
                <select id="id_categoria" name="id_categoria" class="form-select" required>
                    ...
                    <?php foreach ($categorias as $cat): ?>
                        <?php $selected = $cat->getIdCategoria() == $idCategoria ? 'selected' : ''; ?>
            ...
            <div class="mb-3">
                ...
                <select name="autores[]" id="autores" class="form-select" multiple size="6">
                    <?php foreach ($autores as $a): ?>
                        ...
                        $nombreAut = method_exists($a, 'getNombreCompleto')
                            ? $a->getNombreCompleto()
                            : ($a->getNombre() . ' ' . $a->getApellido());
                        ?>
                        <option value="<?= $idAutor ?>" <?= $selected ?>>
                            <?= htmlspecialchars($nombreAut) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                <div class="form-text">
                    Mantén presionada la tecla Ctrl (o Cmd) para seleccionar varios autores.
                </div>
            </div>
